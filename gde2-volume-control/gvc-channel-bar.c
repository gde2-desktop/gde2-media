...
struct _GvcChannelBarPrivate
{
        ...
        Gde2MixerStreamControlFlags control_flags;
};
                             ...
                             GvcChannelBar *bar)
{
        ...
        if (bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_MUTE_WRITABLE)
                gde2_mixer_stream_control_set_mute (bar->priv->control, (value <= lower));

        if (bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_VOLUME_WRITABLE)
                gde2_mixer_stream_control_set_volume (bar->priv->control, (guint) value);
}
...
on_mute_button_toggled (GtkToggleButton *button, GvcChannelBar *bar)
{
        ...
        gde2_mixer_stream_control_set_mute (bar->priv->control, mute);
}
...
update_marks (GvcChannelBar *bar)
{
        ...
        base   = gde2_mixer_stream_control_get_base_volume (bar->priv->control);
        normal = gde2_mixer_stream_control_get_normal_volume (bar->priv->control);
...
update_adjustment_value (GvcChannelBar *bar)
{
        ...
        if (bar->priv->control == NULL)
                set_lower = TRUE;
        else if (bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_MUTE_READABLE)
                set_lower = gde2_mixer_stream_control_get_mute (bar->priv->control);

        if (set_lower == TRUE)
                value = gtk_adjustment_get_lower (bar->priv->adjustment);
        else
                value = gde2_mixer_stream_control_get_volume (bar->priv->control);
...
update_adjustment_limits (GvcChannelBar *bar)
{
        ...
        if (bar->priv->control != NULL) {
                ...
                if (bar->priv->extended)
                        maximum = gde2_mixer_stream_control_get_max_volume (bar->priv->control);
                else
...
update_mute_button (GvcChannelBar *bar)
{
        if (bar->priv->show_mute == TRUE) {
                ...
                if (bar->priv->control != NULL &&
                    bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_MUTE_READABLE)
                        enable = TRUE;

                if (enable == TRUE) {
                        gboolean mute = gde2_mixer_stream_control_get_mute (bar->priv->control);
                             ...
                             GvcChannelBar  *bar)
{
        ...
        if (bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_MUTE_READABLE &&
            bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_VOLUME_READABLE) {
                if (gde2_mixer_stream_control_get_mute (bar->priv->control) == TRUE) {
                        guint minimum = (guint) gtk_adjustment_get_lower (bar->priv->adjustment);

                        if (gde2_mixer_stream_control_get_volume (bar->priv->control) > minimum)
                               ...
                               GvcChannelBar  *bar)
{
        ...
        ca_gtk_play_for_widget (GTK_WIDGET (bar), 0,
                                ...
                                CA_PROP_APPLICATION_ID, "org.gde2.VolumeControl",
...
static void
on_control_volume_notify (Gde2MixerStreamControl *control,
                          GParamSpec             *pspec,
                          GvcChannelBar          *bar)
{
...
static void
on_control_mute_notify (Gde2MixerStreamControl *control,
                        GParamSpec             *pspec,
                        GvcChannelBar          *bar)
{
        if (bar->priv->show_mute == TRUE) {
                gboolean mute = gde2_mixer_stream_control_get_mute (control);
...
void
gvc_channel_bar_set_control (GvcChannelBar *bar, Gde2MixerStreamControl *control)
{
        ...
        if (control != NULL)
                bar->priv->control_flags = gde2_mixer_stream_control_get_flags (control);
        else
                bar->priv->control_flags = GDE2_MIXER_STREAM_CONTROL_NO_FLAGS;

        if (bar->priv->control_flags & GDE2_MIXER_STREAM_CONTROL_VOLUME_READABLE)
...
GtkWidget *
gvc_channel_bar_new (Gde2MixerStreamControl *control)
{
