...
struct _GvcMixerDialogPrivate
{
        ...
        Gde2MixerBackendFlags backend_flags;
        GHashTable       *bars;
        GtkWidget        *notebook;
...
static Gde2MixerSwitch *
find_stream_port_switch (Gde2MixerStream *stream)
{
        const GList *switches;

        switches = gde2_mixer_stream_list_switches (stream);
        while (switches != NULL) {
                Gde2MixerStreamSwitch *swtch = GDE2_MIXER_STREAM_SWITCH (switches->data);

                if (!GDE2_MIXER_IS_STREAM_TOGGLE (swtch) &&
                    gde2_mixer_stream_switch_get_role (swtch) == GDE2_MIXER_STREAM_SWITCH_ROLE_PORT)
                    return GDE2_MIXER_SWITCH (swtch);
...
static Gde2MixerSwitch *
find_device_profile_switch (Gde2MixerDevice *device)
{
        const GList *switches;

        switches = gde2_mixer_device_list_switches (device);
        while (switches != NULL) {
                Gde2MixerDeviceSwitch *swtch = GDE2_MIXER_DEVICE_SWITCH (switches->data);

                if (gde2_mixer_device_switch_get_role (swtch) == GDE2_MIXER_DEVICE_SWITCH_ROLE_PROFILE)
                        return GDE2_MIXER_SWITCH (swtch);
...
static Gde2MixerStream *
find_device_test_stream (GvcMixerDialog *dialog, Gde2MixerDevice *device)
{
        const GList *streams;

        streams = gde2_mixer_device_list_streams (device);
        while (streams != NULL) {
                Gde2MixerStream   *stream;
                Gde2MixerDirection direction;

                stream = GDE2_MIXER_STREAM (streams->data);
                direction = gde2_mixer_stream_get_direction (stream);

                if (direction == GDE2_MIXER_DIRECTION_OUTPUT) {
                    Gde2MixerStreamControl *control;

                    control = gde2_mixer_stream_get_default_control (stream);
                    if (gde2_mixer_stream_control_get_num_channels (control) > 0)
                        return stream;
                }
                          ...
                          Gde2MixerStream *stream)
{
        ...
        if (stream != NULL)
                name = gde2_mixer_stream_get_name (stream);
...
update_output_settings (GvcMixerDialog *dialog)
{
        ...
        flags = gde2_mixer_stream_control_get_flags (control);

        /* Enable balance bar if it is available */
        if (flags & GDE2_MIXER_STREAM_CONTROL_CAN_BALANCE) {
                dialog->priv->output_balance_bar =
        ...
        if (flags & GDE2_MIXER_STREAM_CONTROL_CAN_FADE) {
                dialog->priv->output_fade_bar =
        ...
        if (gde2_mixer_stream_control_has_channel_position (control, GDE2_MIXER_CHANNEL_LFE)) {
                dialog->priv->output_lfe_bar =
        ...
        stream = gde2_mixer_stream_control_get_stream (control);
...
static void
set_output_stream (GvcMixerDialog *dialog, Gde2MixerStream *stream)
{
        GtkTreeModel           *model;
        Gde2MixerSwitch        *swtch;
        Gde2MixerStreamControl *control;
        ...
        if (stream != NULL) {
                ...
                controls = gde2_mixer_context_list_stored_controls (dialog->priv->context);

                /* Move all stored controls to the newly selected default stream */
                while (controls != NULL) {
                        Gde2MixerStream        *parent;
                        Gde2MixerStreamControl *control;

                        control = GDE2_MIXER_STREAM_CONTROL (controls->data);
                        parent  = gde2_mixer_stream_control_get_stream (control);
                        ...
                        if (parent != NULL && parent != stream) {
                                Gde2MixerDirection direction =
                                        gde2_mixer_stream_get_direction (parent);

                                if (direction == GDE2_MIXER_DIRECTION_OUTPUT)
                                        gde2_mixer_stream_control_set_stream (control, stream);
                        }
...
static void
on_context_default_output_stream_notify (Gde2MixerContext *context,
                                         GParamSpec       *pspec,
                                         GvcMixerDialog   *dialog)
{
        Gde2MixerStream *stream;

        stream = gde2_mixer_context_get_default_output_stream (context);
...
static void
on_stream_control_monitor_value (Gde2MixerStream *stream,
                                 gdouble          value,
                                 GvcMixerDialog  *dialog)
{
        GtkAdjustment *adj;
...
update_input_settings (GvcMixerDialog *dialog)
{
        ...
        if (flags & GDE2_MIXER_STREAM_CONTROL_HAS_MONITOR)
                               ...
                               GvcMixerDialog         *dialog)
{
        ...
        if (gde2_mixer_stream_control_get_mute (control) == TRUE)
                gde2_mixer_stream_control_set_monitor_enabled (control, FALSE);
        else
                gde2_mixer_stream_control_set_monitor_enabled (control, TRUE);
}

static void
set_input_stream (GvcMixerDialog *dialog, Gde2MixerStream *stream)
{
        GtkTreeModel           *model;
        Gde2MixerSwitch        *swtch;
        Gde2MixerStreamControl *control;
        ...
        if (stream != NULL) {
                ...
                while (controls != NULL) {
                        ...
                        if (parent != NULL && parent != stream) {
                                ...
                                if (direction == GDE2_MIXER_DIRECTION_INPUT)
                                        gde2_mixer_stream_control_set_stream (control, stream);
                        }
                        controls = controls->next;
                }
...
static void
on_context_default_input_stream_notify (Gde2MixerContext *context,
                                        GParamSpec       *pspec,
                                        GvcMixerDialog   *dialog)
{
        Gde2MixerStream *stream;
        ...
        stream = gde2_mixer_context_get_default_input_stream (context);
...
bar_set_stream (GvcMixerDialog  *dialog,
                ...
                Gde2MixerStream *stream)
{
        Gde2MixerStreamControl *control = NULL;
...
{
        const gchar            *name;
        Gde2MixerStreamControl *previous;
        ...
        if (previous != NULL) {
                name = gde2_mixer_stream_control_get_name (previous);
                ...
                gde2_mixer_stream_control_set_monitor_enabled (previous, FALSE);
        ...
        if (control != NULL) {
                name = gde2_mixer_stream_control_get_name (control);
...
static void
add_application_control (GvcMixerDialog *dialog, Gde2MixerStreamControl *control)
{
        Gde2MixerStream                *stream;
        Gde2MixerStreamControlMediaRole media_role;
        Gde2MixerAppInfo               *info;
        Gde2MixerDirection              direction = GDE2_MIXER_DIRECTION_UNKNOWN;
        GtkWidget                      *bar;
        const gchar                    *app_id;
        const gchar                    *app_name;
        const gchar                    *app_icon;

        media_role = gde2_mixer_stream_control_get_media_role (control);

        /* Add stream to the applications page, but make sure the stream qualifies
         * for the inclusion */
        info = gde2_mixer_stream_control_get_app_info (control);
        if (info == NULL)
                return;

        /* Skip streams with roles we don't care about */
        if (media_role == GDE2_MIXER_STREAM_CONTROL_MEDIA_ROLE_EVENT ||
            media_role == GDE2_MIXER_STREAM_CONTROL_MEDIA_ROLE_TEST ||
            media_role == GDE2_MIXER_STREAM_CONTROL_MEDIA_ROLE_ABSTRACT ||
            media_role == GDE2_MIXER_STREAM_CONTROL_MEDIA_ROLE_FILTER)
                return;

        app_id = gde2_mixer_app_info_get_id (info);

        /* These applications may have associated streams because they do peak
         * level monitoring, skip these too */
        if (!g_strcmp0 (app_id, "org.gde2.VolumeControl") ||
            !g_strcmp0 (app_id, "org.gnome.VolumeControl") ||
            !g_strcmp0 (app_id, "org.PulseAudio.pavucontrol"))
                return;

        app_name = gde2_mixer_app_info_get_name (info);
        if (app_name == NULL)
                app_name = gde2_mixer_stream_control_get_label (control);
        if (app_name == NULL)
                app_name = gde2_mixer_stream_control_get_name (control);
        ...
        app_icon = gde2_mixer_app_info_get_icon (info);
        if (app_icon == NULL) {
                if (direction == GDE2_MIXER_DIRECTION_INPUT)
...
static void
on_stream_control_added (Gde2MixerStream *stream,
                         const gchar     *name,
                         GvcMixerDialog  *dialog)
{
        Gde2MixerStreamControl    *control;
        Gde2MixerStreamControlRole role;

        control = gde2_mixer_stream_get_control (stream, name);
        if G_UNLIKELY (control == NULL)
                return;

        role = gde2_mixer_stream_control_get_role (control);

        if (role == GDE2_MIXER_STREAM_CONTROL_ROLE_APPLICATION)
                add_application_control (dialog, control);
}

static void
on_stream_control_removed (Gde2MixerStream *stream,
                           const gchar     *name,
                           GvcMixerDialog  *dialog)
{
        Gde2MixerStreamControl *control;
        ...
        control = gvc_channel_bar_get_control (GVC_CHANNEL_BAR (dialog->priv->input_bar));
        if (control != NULL) {
                const gchar *input_name = gde2_mixer_stream_control_get_name (control);
...
static void
add_stream (GvcMixerDialog *dialog, Gde2MixerStream *stream)
{
        ...
        if (direction == GDE2_MIXER_DIRECTION_INPUT) {
                Gde2MixerStream *input;

                input = gde2_mixer_context_get_default_input_stream (dialog->priv->context);
                if (stream == input) {
                        bar_set_stream (dialog, dialog->priv->input_bar, stream);
        ...
        else if (direction == GDE2_MIXER_DIRECTION_OUTPUT) {
                Gde2MixerStream        *output;
                Gde2MixerStreamControl *control;

                output = gde2_mixer_context_get_default_output_stream (dialog->priv->context);
                if (stream == output) {
                        bar_set_stream (dialog, dialog->priv->output_bar, stream);
        ...
        controls = gde2_mixer_stream_list_controls (stream);
        while (controls != NULL) {
                Gde2MixerStreamControl    *control = GDE2_MIXER_STREAM_CONTROL (controls->data);
                Gde2MixerStreamControlRole role;
        ...
        if (model != NULL) {
                ...
                name  = gde2_mixer_stream_get_name (stream);
                label = gde2_mixer_stream_get_label (stream);
...
static void
on_context_stream_added (Gde2MixerContext *context,
                         const gchar      *name,
                         GvcMixerDialog   *dialog)
{
        Gde2MixerStream   *stream;
        ...
        stream = gde2_mixer_context_get_stream (context, name);
        if (G_UNLIKELY (stream == NULL))
                return;
        ...
        if (dialog->priv->hw_profile_combo != NULL && direction == GDE2_MIXER_DIRECTION_OUTPUT) {
                Gde2MixerDevice *device1;
                Gde2MixerDevice *device2;

                device1 = gde2_mixer_stream_get_device (stream);
...
static void
on_context_stream_removed (Gde2MixerContext *context,
                           const gchar      *name,
                           GvcMixerDialog   *dialog)
{
...
static void
on_context_stored_control_added (Gde2MixerContext *context,
                                 const gchar      *name,
                                 GvcMixerDialog   *dialog)
{
        Gde2MixerStreamControl         *control;
        ...
        control = GDE2_MIXER_STREAM_CONTROL (gde2_mixer_context_get_stored_control (context, name));
        if (G_UNLIKELY (control == NULL))
                return;
...
static void
on_context_stored_control_removed (Gde2MixerContext *context,
                                   const gchar      *name,
                                   GvcMixerDialog   *dialog)
{
        GtkWidget *bar;
...
static gchar *
device_status (Gde2MixerDevice *device)
{
        guint        inputs = 0;
        guint        outputs = 0;
        ...
        streams = gde2_mixer_device_list_streams (device);
        while (streams != NULL) {
                Gde2MixerStream   *stream = GDE2_MIXER_STREAM (streams->data);
                Gde2MixerDirection direction;
...
static void
update_device_info (GvcMixerDialog *dialog, Gde2MixerDevice *device)
{
        GtkTreeModel    *model = NULL;
        GtkTreeIter      iter;
        const gchar     *label;
        ...
        gchar           *status;
        Gde2MixerSwitch *profile_switch;

        model = gtk_tree_view_get_model (GTK_TREE_VIEW (dialog->priv->hw_treeview));

        if (find_tree_item_by_name (model,
                                    gde2_mixer_device_get_name (device),
                                    HW_NAME_COLUMN,
                                    &iter) == FALSE)
                return;

        label = gde2_mixer_device_get_label (device);

        profile_switch = find_device_profile_switch (device);
        if (profile_switch != NULL) {
                Gde2MixerSwitchOption *active;

                active = gde2_mixer_switch_get_active_option (profile_switch);
                if (G_LIKELY (active != NULL))
                        profile_label = gde2_mixer_switch_option_get_label (active);
        }
...
static void
on_device_profile_active_option_notify (Gde2MixerDeviceSwitch *swtch,
                                        GParamSpec            *pspec,
                                        GvcMixerDialog        *dialog)
{
        Gde2MixerDevice *device;

        device = gde2_mixer_device_switch_get_device (swtch);

        update_device_info (dialog, device);
}

static void
add_device (GvcMixerDialog *dialog, Gde2MixerDevice *device)
{
        GtkTreeModel    *model;
        GtkTreeIter      iter;
        GIcon           *icon;
        ...
        name  = gde2_mixer_device_get_name (device);
        label = gde2_mixer_device_get_label (device);
        ...
        icon = g_themed_icon_new_with_default_fallbacks (gde2_mixer_device_get_icon (device));
...
static void
on_context_device_added (Gde2MixerContext *context, const gchar *name, GvcMixerDialog *dialog)
{
        Gde2MixerDevice *device;

        device = gde2_mixer_context_get_device (context, name);
        if (G_UNLIKELY (device == NULL))
                return;
...
static void
on_context_device_removed (Gde2MixerContext *context,
                           const gchar      *name,
                           GvcMixerDialog   *dialog)
{
        GtkTreeIter   iter;
                        ...
                        GvcMixerDialog        *dialog)
{
        ...
        if (toggled ^ 1) {
                Gde2MixerStream      *stream;
                Gde2MixerBackendFlags flags;

                stream = gde2_mixer_context_get_stream (dialog->priv->context, name);
                if (G_UNLIKELY (stream == NULL)) {
                        g_warn_if_reached ();
                        g_free (name);
                        return;
                ...
                // XXX cache this
                flags = gde2_mixer_context_get_backend_flags (dialog->priv->context);

                if (flags & GDE2_MIXER_BACKEND_CAN_SET_DEFAULT_INPUT_STREAM)
                        gde2_mixer_context_set_default_input_stream (dialog->priv->context, stream);
                else
                        set_input_stream (dialog, stream);
        }
        g_free (name);
                         ...
                         GvcMixerDialog        *dialog)
{
        ...
        if (toggled ^ 1) {
                ...
                if (flags & GDE2_MIXER_BACKEND_CAN_SET_DEFAULT_OUTPUT_STREAM)
                        gde2_mixer_context_set_default_output_stream (dialog->priv->context, stream);
                else
                        set_output_stream (dialog, stream);
        }
        g_free (name);
...
on_test_speakers_clicked (GvcComboBox *widget, GvcMixerDialog *dialog)
{
        ...
        title = g_strdup_printf (_("Speaker Testing for %s"),
                                 gde2_mixer_device_get_label (device));
...
on_device_selection_changed (GtkTreeSelection *selection, GvcMixerDialog *dialog)
{
        ...
        device = gde2_mixer_context_get_device (dialog->priv->context, name);
        if (G_UNLIKELY (device == NULL)) {
                g_warn_if_reached ();
                g_free (name);
                return;
...
create_page_effects (GvcMixerDialog *self)
{
         ...
         */
        if (self->priv->backend_flags & GDE2_MIXER_BACKEND_HAS_STORED_CONTROLS) {
                GvcChannelBar *bar;
                const GList   *list;
                ...
                list = gde2_mixer_context_list_stored_controls (self->priv->context);
                while (list != NULL) {
                        Gde2MixerStreamControl *control = GDE2_MIXER_STREAM_CONTROL (list->data);
                        Gde2MixerStreamControlMediaRole media_role;
                              ...
                              GObjectConstructParam *construct_params)
{
        ...
        list = gde2_mixer_context_list_streams (self->priv->context);
        while (list != NULL) {
                add_stream (self, GDE2_MIXER_STREAM (list->data));
                list = list->next;
        }

        list = gde2_mixer_context_list_devices (self->priv->context);
        while (list != NULL) {
                add_device (self, GDE2_MIXER_DEVICE (list->data));
                list = list->next;
        }
...
static void
gvc_mixer_dialog_set_context (GvcMixerDialog *dialog, Gde2MixerContext *context)
{
        ...
        dialog->priv->backend_flags = gde2_mixer_context_get_backend_flags (context);
...
GvcMixerDialog *
gvc_mixer_dialog_new (Gde2MixerContext *context)
{
